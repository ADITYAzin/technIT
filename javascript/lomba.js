...
option.forEach((o, i) => {
  o.addEventListener('click', () => {
    // Hapus kelas yang diinginkan dari semua tombol sebelum menambahkan kelas pada yang dipilih
    option.forEach((btn) => {
      btn.classList.remove('bg-blue-950');
      btn.classList.remove('text-white');
      btn.classList.add('text-blue-950');
      btn.classList.add('bg-white');  // Menetapkan warna default pada semua tombol
    });

    // Tambahkan kelas pada tombol yang diklik
    o.classList.remove('bg-white'); // Menghapus warna default
    o.classList.add('bg-blue-950'); // Memberikan warna baru pada tombol yang diklik
    o.classList.remove('text-blue-950'); // Memberikan warna baru pada tombol yang diklik
    o.classList.add('text-white'); // Memberikan warna baru pada tombol yang diklik

    switch(i) {
      case 0:
        fetchYTChannelWebDev();  // Fungsi untuk tombol pertama
        break;
      case 1:
        fetchYTChannelNetwoking();  // Fungsi untuk tombol kedua
        break;
      case 2:
        fetchYTChannelCyberSecurity();    // Fungsi untuk tombol ketiga
        break;
      case 3:
        fetchYTChannelDesign();    // Fungsi untuk tombol ketiga
        break;
      default:
...
//Fetch Data Channel Area

function fetchYTChannelWebDev(){
  fetch('data/channelWebDev.json')
  ...
  .then(data => {
    ...
    data.forEach(e => {
      const CardsChannel = UICardYTChannelWebDev(e); // Memanggil fungsi UICardYTChannel untuk setiap elemen data
    ...
    swiperSlides.forEach((sS, index) => {
      ...
      if (index < data.length) {
        sS.innerHTML = UICardYTChannelWebDev(data[index]); // Setiap swiper-slide diisi dengan card yang sesuai
...
function fetchYTChannelNetwoking(){
  fetch('data/channelNetworking.json')
  .then(response => response.json()) // Mengambil data JSON
  .then(data => {
    let cardsChannel = '';
    ...
    // Loop untuk menambahkan setiap channel ke dalam cardsChannel
    data.forEach(e => {
      const CardsChannel = UICardYTChannelNetworking(e); // Memanggil fungsi UICardYTChannel untuk setiap elemen data
      cardsChannel += CardsChannel; // Gabungkan HTML card ke dalam string
    });
    ...
    swiperSlides.forEach((sS, index) => {
      // Pastikan index sesuai dengan data yang ada
      if (index < data.length) {
        sS.innerHTML = UICardYTChannelNetworking(data[index]); // Setiap swiper-slide diisi dengan card yang sesuai
...
function fetchYTChannelCyberSecurity(){
  fetch('data/channelCyberSecurity.json')
  .then(response => response.json()) // Mengambil data JSON
  .then(data => {
    let cardsChannel = '';
    ...
    // Loop untuk menambahkan setiap channel ke dalam cardsChannel
    data.forEach(e => {
      const CardsChannel = UICardYTChannelCyberSecurity(e); // Memanggil fungsi UICardYTChannel untuk setiap elemen data
      cardsChannel += CardsChannel; // Gabungkan HTML card ke dalam string
    });
    ...
    swiperSlides.forEach((sS, index) => {
      // Pastikan index sesuai dengan data yang ada
      if (index < data.length) {
        sS.innerHTML = UICardYTChannelCyberSecurity(data[index]); // Setiap swiper-slide diisi dengan card yang sesuai
...
function fetchYTChannelDesign(){
  fetch('data/channelDesain.json')
  .then(response => response.json()) // Mengambil data JSON
  .then(data => {
    let cardsChannel = '';
    ...
    // Loop untuk menambahkan setiap channel ke dalam cardsChannel
    data.forEach(e => {
      const CardsChannel = UICardYTChannelDesign(e); // Memanggil fungsi UICardYTChannel untuk setiap elemen data
      cardsChannel += CardsChannel; // Gabungkan HTML card ke dalam string
    });
    ...
    swiperSlides.forEach((sS, index) => {
      // Pastikan index sesuai dengan data yang ada
      if (index < data.length) {
        sS.innerHTML = UICardYTChannelDesign(data[index]); // Setiap swiper-slide diisi dengan card yang sesuai
...
fetchYTChannelWebDev();

//UI Card YT Channel Area


function UICardYTChannelWebDev(applies,i) {
  return `
    <div class="card block bg-white p-4 w-[360px] rounded-[2.5rem] overflow-hidden h-[86px] scale-75" style="box-shadow: rgba(99, 99, 99, 0.2) 0px 2px 8px 0px;">
      <div class="card-content w-full flex flex-col items-center justify-between h-full gap-4">
        <div class="card-header w-full flex flex-row items-center justify-between">
        ...
        <div class="middle-card-area w-full h-[84px] overflow-y-scroll text-[14px]">
        ...
        <div class="socials w-full flex flex-col items-center justify-center gap-4">
          <div class="socials-icon flex flex-row items-center justify center gap-2">
            <a href="${applies.github}"><svg xmlns="http://www.w3.org/2000/svg" width="1.5em" height="1.5em" viewBox="0 0 24 24">
            ...
            <a href="${applies.instagram}"><svg xmlns="http://www.w3.org/2000/svg" width="1.5em" height="1.5em" viewBox="0 0 24 24">
...
function UICardYTChannelNetworking(applies,i) {
  return `
    <div class="card block bg-white p-4 w-[360px] rounded-[2.5rem] overflow-hidden h-[86px] scale-75" style="box-shadow: rgba(99, 99, 99, 0.2) 0px 2px 8px 0px;">
      <div class="card-content w-full flex flex-col items-center justify-between h-full gap-4">
        <div class="card-header w-full flex flex-row items-center justify-between">
